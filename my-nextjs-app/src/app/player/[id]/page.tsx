      ...
      fetch(`https://us-central1-summaristt.cloudfunctions.net/getBook?id=${id}`)
        ...
        .then(data => {
          ...
          if (data.subscriptionRequired) {
            ...
            // If not logged in, redirect to for-you (Sidebar will handle login)
            if (!loggedIn) {
            ...
            // If logged in but not subscribed, redirect to choose plan
            if (!subscribed) {
              setIntendedDestination(`/player/${id}`);
              setLoading(false);
          ...
          // Initialize audio - only if user has access
          if (data.audioLink) {
